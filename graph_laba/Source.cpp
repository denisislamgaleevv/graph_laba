...
Color = vec4(clamp(Position, 0.5, 1.0), 0.1);                                 \n\
}";
...
public:
	...
	void InitRotateTransform(glm::mat4& m) const
	{
		...
		rx[1][0] = 0.0f; rx[1][1] = sinf(x); rx[1][2] = -sinf(x); rx[1][3] = 0.0f;
		rx[2][0] = 0.0f; rx[2][1] = cosf(x); rx[2][2] = cosf(x); rx[2][3] = 0.0f;
...
{
	glm::vec3 Vertices[3];
	Vertices[0] = glm::vec3(-0.5f, -1.0f, 0.0f);
